...
export default function MemberResources() {
  ...
  useEffect(() => {
    const fetchResources = async () => {
      try {
        ...
        const response = await fetch('/api/youtube/church-content?maxResults=50');

        if (response.ok) {
          const data = await response.json();

          // Transform YouTube videos into resources
          const youtubeResources: Resource[] = data.videos?.map((video: any, index: number) => ({
            id: video.id,
            title: video.title,
  ...
  const determineCategory = (title: string, description: string): string => {
    ...
    if (content.includes('sermon') || content.includes('message') || content.includes('preaching')) return 'Sermons';
    if (content.includes('prayer') || content.includes('intercession')) return 'Prayer';
    if (content.includes('worship') || content.includes('song') || content.includes('music')) return 'Music';
    if (content.includes('study') || content.includes('bible')) return 'Bible Study';
    if (content.includes('youth') || content.includes('children')) return 'Children';
    if (content.includes('testimony') || content.includes('testimonies')) return 'Testimonies';
  ...
  const getTypeColor = (type: string) => {
    const colors = {
      video: "bg-church-primary/10 text-church-primary border-church-primary/20",
      sermon: "bg-church-accent/10 text-church-accent border-church-accent/20",
      study: "bg-church-primary/10 text-church-primary border-church-primary/20",
      prayer: "bg-church-accent/10 text-church-accent border-church-accent/20",
      music: "bg-church-primary/10 text-church-primary border-church-primary/20",
      document: "bg-church-accent/10 text-church-accent border-church-accent/20",
      curriculum: "bg-church-primary/10 text-church-primary border-church-primary/20",
    };
  ...
  return (
    <div className="container max-w-6xl py-10 space-y-8 bg-church-surface">
      {/* Header */}
      <div>
        <h1 className="text-xl md:text-2xl font-bold text-church-primary">Member Resources</h1>
      ...
      {resources.filter(r => r.featured).length > 0 && (
        <div>
          <h2 className="text-xl font-semibold text-church-text mb-4">Featured Resources</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {resources.filter(r => r.featured).map((resource) => {
              const Icon = getTypeIcon(resource.type);
              return (
                <Card key={resource.id} className="hover:shadow-lg transition-all duration-300 border-l-4 border-church-accent group overflow-hidden">
                  {resource.thumbnail && (
                  ...
                  <CardHeader className="pb-3">
                    ...
                    <CardTitle className="text-lg leading-tight text-church-text">{resource.title}</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    ...
                    <div className="flex items-center justify-between text-xs text-church-text-muted">
                      ...
                      {resource.viewCount && <span>{resource.viewCount} views</span>}
                    </div>
        ...
        <CardContent className="pt-6">
          <div className="flex flex-col lg:flex-row gap-4">
            <div className="flex-1">
              <Label htmlFor="search" className="text-church-text">Search Resources</Label>
            ...
            <div className="w-full lg:w-48">
              <Label htmlFor="category" className="text-church-text">Category</Label>
              <select
                id="category"
            ...
            <div className="w-full lg:w-48">
              <Label htmlFor="sort" className="text-church-text">Sort By</Label>
              <select
                id="sort"
                value={sortBy}
      ...
      {/* All Resources */}
      <div>
        <h2 className="text-xl font-semibold text-church-text mb-4">All Resources</h2>
        <div className="space-y-4">
          {filteredResources.map((resource) => {
            const Icon = getTypeIcon(resource.type);
            return (
              <Card key={resource.id} className="hover:shadow-lg transition-all duration-300 border-church-primary/20 group">
                <CardContent className="pt-6">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between mb-2 gap-4">
                        <div className="flex-1">
                          <h3 className="font-semibold text-lg text-church-text group-hover:text-church-accent transition-colors">{resource.title}</h3>
                          <p className="text-sm text-church-text-muted mt-1 line-clamp-2">
                            {resource.description}
                          </p>
                        </div>
                        <Badge className={`${getTypeColor(resource.type)} border flex-shrink-0`}>
                          {resource.category}
                        </Badge>
                      </div>
      ...
      {filteredResources.length === 0 && (
        <Card className="border-church-primary/20">
          <CardContent className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold mb-2 text-church-text">No resources found</h3>
