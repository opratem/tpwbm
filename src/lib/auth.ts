      ...
      try {
        ...
        if (user?.email) {
          ...
          if (dbUser) {
            ...
            let userName = 'User';
            if (dbUser.name && typeof dbUser.name === 'string' && dbUser.name.trim()) {
              userName = dbUser.name.trim();
            } else if (dbUser.email && typeof dbUser.email === 'string') {
    ...
    async session({ session, token }) {
      try {
        if (session.user && token) {
          ...
          let userName = 'User';
          if (token.name && typeof token.name === 'string' && (token.name as string).trim()) {
            userName = (token.name as string).trim();
          } else if (token.email && typeof token.email === 'string') {
            const emailParts = (token.email as string).split('@');
            if (emailParts.length > 0 && emailParts[0]) {
              userName = emailParts[0];
            }
          }
